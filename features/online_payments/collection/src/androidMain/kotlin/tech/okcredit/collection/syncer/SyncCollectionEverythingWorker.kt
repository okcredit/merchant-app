...
class SyncCollectionEverythingWorker(
    context: Context,
    ...
    override suspend fun doActualWork() {
        val businessId = inputData.getString(CollectionSyncer.BUSINESS_ID)!!
        val source = inputData.getString("SOURCE") ?: "unknown"
        syncer.value.syncAll(businessId, source)
    }

    @Inject
    class Factory(private val syncer: Lazy<CollectionSyncer>) : ChildWorkerFactory {
        override fun create(context: Context, params: WorkerParameters): ListenableWorker {
            return SyncCollectionEverythingWorker(context, params, syncer)
        }
    }
}
